...
def preprocess_dataset_with_answers(
    ...
    context_list_column_name = "context_list" if "context" in column_names else column_names[1]
    ...
    def prepare_features(examples):
        ...
        # cls+ question 1 + sep + passage1 +' ' + passage2 + ' ' +passage3 ...
        # cls+ question 1 + sep 까지의 길이 / passage1+ ' ' 길이/ pssage2+ ' '길이 -> 또 토크나이징이 되니까
        ...
        tokenized_examples1 = tokenizer(
            examples[question_column_name if pad_on_right else context_column_name]
        )


        tokenized_examples2 = tokenizer(
            examples[context_list_column_name if pad_on_right else context_column_name]
        )
        ...
        print('inside preprocessing with answers')
        breakpoint()
        for i, offsets in enumerate(offset_mapping):
            input_ids = tokenized_examples["input_ids"][i] # input_ids == 단어들 token value
            cls_index = input_ids.index(tokenizer.cls_token_id)  # cls index

            # sequence id를 설정합니다 (to know what is the context and what is the question).
            # [None, 0, 0, 0, 0, 0, None, 1, 1, 1, 1, 1, None] -> None은 CLS 또는 SEP
