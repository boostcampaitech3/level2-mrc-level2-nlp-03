...
from transformers import AutoTokenizer, BertModel, BertPreTrainedModel, AdamW, TrainingArguments, \
...
class DenseRetrieval(SparseRetrieval):
    ...
    def __init__(self, tokenize_fn, data_path, context_path, dataset_path, tokenizer, train_data, num_neg,
                 is_bm25=False, wandb=False):
    ...
    def make_train_data(self, tokenizer):
        ...
        top_k = self.num_neg * 10 # 이러면 120개인데

        if self.is_bm25 == True:
            global retriever
            retriever = self
            doc_scores, doc_indices = par_search(queries, top_k) # 왜 120개 search 하는거지
        else:
            query_vec = self.tfidfv.transform(queries)
            doc_scores, doc_indices = self.get_topk_similarity(query_vec, top_k) #topk 120dlsep docindices length 확인 필요

        neg_idxs = []
        for idx, ind in enumerate(tqdm(doc_indices)):  # 4000 -> 3952갠데?
            neg_idx = []
            for i in range(len(ind)):  # 2~20 find negative
                if not self.contexts[ind[i]][:10] in self.train_data['context'][idx]: #여기도 이해안됨
    ...
    def train(self, args, dataset):
        ...
        train_iterator = trange(5, desc="Epoch")
        for epoch, _ in enumerate(train_iterator):
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                p_outputs = torch.transpose(p_outputs.view(args.per_device_train_batch_size, self.num_neg + 1, -1), 1,
                                            2)
    ...
    def get_relevant_doc_bulk(self, queries: List, k: Optional[int] = 1) -> Tuple[List, List]:
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(dataloader):
                q_seqs_val = self.tokenizer(batch, padding="max_length", truncation=True, return_tensors='pt').to(
                    'cuda')
    ...
    def topk_experiment(self, topK_list, dataset, datatset_name="train"):
        ...
        result_dict = {} # topk 중 가장 큰값으로 먼저 뽑고 그 안에서 indexing 하면 계산 줄이기 가능
        result_retriever = self.retrieve(dataset, topk=max(topK_list))

        for topK in tqdm(topK_list):
            correct = 0
            for index in tqdm(range(len(result_retriever)), desc="topk_experiment"): # 이것도 위에서 한번에 sorting reverse해주면 될 것 같은데
                # 이 200의 정체가 무엇인가..
...
if __name__ == "__main__":
    ...
    ## 추론준비 ##
    dense_retriever.load_model(model_checkpoint, "./outputs/dpr/p_encoder_3.pt", "./outputs/dpr/q_encoder_3.pt")

    dense_retriever.get_dense_embedding()
    # with open("/opt/ml/input/data/dense_embedding.bin", "rb") as f: # dense_embedding 한번 실행후 진행
