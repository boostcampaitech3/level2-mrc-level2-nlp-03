...
class SparseRetrieval:
    def __init__(
        ...
        self.p_embedding = None #self.get_sparse_embedding()  # get_sparse_embedding()로 생성합니다
        ...
        if isinstance(query_or_dataset, str):
            ...
            for idx, example in enumerate(
                ...
                tmp = {
                    ...
                    # 여기서 이렇게 합쳐버리면 나중에 scoring할 때 안된다.
                    # "context": " ".join(
...
if __name__ == "__main__":
    ...
    if args.use_faiss:
        ...
        with timer("bulk query by exhaustive search"):
            # topk 성능 테스트를 위한 코드로 변경
            # topk를 100정도로 여유있게 뽑고

            df = retriever.retrieve(full_ds, topk=100)
            test_topk_list = [1,2,5,10,20]
            for topk_test in test_topk_list:
                test = [0] * len(df)
                for test_idx in range(len(df['original_context'])):
                    if df['original_context'][test_idx] in df['context'][test_idx][:topk_test]:
                        test[test_idx] += 1

                df[f'correct_{topk_test}'] = test
                # df["correct"] = df["original_context"] == df["context"]
                print(
                    f"correct retrieval by {topk_test} result by exhaustive search",
                    df[f'correct_{topk_test}'].sum() / len(df),
                )

            print('#########')
            """ original + topk 한번 수행할 수 있는 코드
            df = retriever.retrieve(full_ds, topk=5)
            test = [0]*len(df)
            for test_idx in range(len(df['original_context'])):
                if df['original_context'][test_idx] in df['context'][test_idx]:
                    test[test_idx] += 1
            breakpoint()
            df['correct'] = test
            # df["correct"] = df["original_context"] == df["context"]
