...
class DenseRetrieval(SparseRetrieval):
    ...
    def train(self, args, dataset):
        ...
        for epoch, _ in enumerate(train_iterator):
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                q_outputs = self.p_encoder(**q_inputs)  # (batch_size*, emb_dim)
                ...
                sim_scores = sim_scores.view(args.per_device_train_batch_size, -1) #TODO :  CONTRASTIVE LOSS
    ...
    def get_relevant_doc(self, query: str, k: Optional[int] = 1) -> Tuple[List, List]:
        ...
        with torch.no_grad():
            ...
            q_emb = self.p_encoder(**q_seqs_val).to('cpu')  # (num_query, emb_dim)
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(dataloader):
                q_seqs_val = self.tokenizer(batch, padding="max_length", truncation=True, return_tensors='pt').to(
                    'cuda')
                q_emb = self.p_encoder(**q_seqs_val).to('cpu')
    ...
    def topk_experiment(self, topK_list, dataset, datatset_name="train"):
        ...
        for topK in tqdm(topK_list):
            ...
            result_retriever = self.retrieve_dpr(dataset=dataset, topk=topK, p_encoder=self.p_encoder,
