...
class DenseRetrieval(SparseRetrieval):
    ...
    def init_model(self, i):
        """ Encoder 모델을 생성해 줍니다."""
        print("init_model...")
        self.p_encoder = BertEncoder.from_pretrained(f"./outputs/dpr/p_encoder_{i}").cuda()
        self.q_encoder = BertEncoder.from_pretrained(f"./outputs/dpr/q_encoder_{i}").cuda()
